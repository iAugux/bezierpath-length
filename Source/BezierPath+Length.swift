...
private extension CGPath {
	...
	func point(at percent: CGFloat, with elements: [PathElement]) -> CGPoint? {
		...
		for e in elements {
			...
			case let .move(to: p0):
				currentPoint = p0
				if firstPointInSubpath == nil { firstPointInSubpath = p0 }
				break
...
private extension CGPath {
    ...
    func apply(with applier: @escaping PathApplier) {
        ...
        apply(info: unsafeBitCast(applier, to: UnsafeMutableRawPointer.self), function: unsafeBitCast(callback, to: CGPathApplierFunction.self))
    }

    var elements: [PathElement] {
        var pathElements = [PathElement]()
        apply {
            let pathElement = PathElement(element: $0.pointee)
